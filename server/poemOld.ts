...
class Connection {
  ...
  requestEditorActivity() {
    ...
    this.io.to(this.socket.id).emit("editorActive", hasPoemInQueue);
  }
...
class Editor extends Member {
  ...
  constructor(io: Server<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>,
              ...
              deviceID: string,
              name: string) {
    super(io, hostSocket, room, deviceID, name);
...
// addHostListeners
// setHostOutgoingMessages
// this makes sure only the necessary messages are being passed

// then re-do everything again lol
...
function poem(io: Server<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("createGameHost", (roomName) => {
      console.log("createGameHost for ", socket.id, "joining", roomName);
      socket.join(roomName);
      roomHost.set(roomName, socket);
      console.log("room", roomName, "created with ", socket.id, "as host");
      const room = new Room(io, socket, roomName)
      roomIDToRoom.set(roomName, room);
    });

    // 1) recognize device
    // 2) check whether coming from host or editor/spectator
    //    if editor/spectator, enforce that they can only have one device
    //      (connect new socket to old info and delete old socket)
    // 3) if host, create Room instance
    //    if editor/spectator, check if join info is valid (room exists, space for editor)
    //      if join invalid, toss back error message to join screen
    //      if join valid, create Editor/Spectator instance and have Room consume it
